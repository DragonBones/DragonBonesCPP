...
namespace dragonBones
{
        ...
        {
            cocos2d::Node *parent = 0;
            int zOrder = 0;
            if(mDisplayNode)
            {
                parent = mDisplayNode->node->getParent();
                zOrder = mDisplayNode->node->getLocalZOrder();
        ...
        void Cocos2dxDisplayBridge::updateTransform(Matrix &matrix, DBTransform &transform)
        {
            if(mDisplayNode)
            {
                cocos2d::Node *n = mDisplayNode->node;
                n->setPosition(matrix.tx , -matrix.ty);
                n->setScaleX(transform.scaleX);
                n->setScaleY(transform.scaleY);
                n->setRotationSkewX(CC_RADIANS_TO_DEGREES(transform.skewX));
                n->setRotationSkewY(CC_RADIANS_TO_DEGREES(transform.skewY));
            }
        }
        ...
        )
        {
            ...
            {
                cocos2d::Node *n = mDisplayNode->node;
                n->setColor(cocos2d::Color3B(rMultiplier * 255 , gMultiplier * 255 , bMultiplier * 255));
