...
MeshData * JSONDataParser::_parseMesh(const rapidjson::Value & rawData)
{
	...
	for (std::size_t i = 0, iW = 0, l = rawVertices.Size(); i < l; i += 2)
	{
		...
		if (mesh->skinned)
		{
			...
			mesh->slotPose.transformPoint(x, y, helpPoint);
			...
			for (std::size_t iB = 0; iB < numBones; ++iB)
			{
				...
				mesh->inverseBindPose[boneIndex].transformPoint(x, y, helpPoint);

				indices.push_back(boneIndex);
				weights.push_back((*rawWeights)[iI + 1].GetFloat());
				boneVertices.push_back(helpPoint.x);
				boneVertices.push_back(helpPoint.y);
			}
...
AnimationData * JSONDataParser::_parseAnimation(const rapidjson::Value & rawData)
{
	...
	_parseTimeline(rawData, *animation, std::bind(&JSONDataParser::_parseAnimationFrame, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
BoneTimelineData * JSONDataParser::_parseBoneTimeline(const rapidjson::Value & rawData) const
{
    ...
    _parseTimeline(rawData, *timeline, std::bind(&JSONDataParser::_parseBoneFrame, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
SlotTimelineData * JSONDataParser::_parseSlotTimeline(const rapidjson::Value & rawData) const
{
    ...
    _parseTimeline(rawData, *timeline, std::bind(&JSONDataParser::_parseSlotFrame, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
FFDTimelineData * JSONDataParser::_parseFFDTimeline(const rapidjson::Value & rawData)
{
    ...
    _parseTimeline(rawData, *timeline, std::bind(&JSONDataParser::_parseFFDFrame, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
FrameData * JSONDataParser::_parseAnimationFrame(const rapidjson::Value& rawData, unsigned frameStart, unsigned frameCount) const
{
...
FrameData * JSONDataParser::_parseBoneFrame(const rapidjson::Value& rawData, unsigned frameStart, unsigned frameCount) const
{
...
FrameData * JSONDataParser::_parseSlotFrame(const rapidjson::Value & rawData, unsigned frameStart, unsigned frameCount) const
{
...
FrameData * JSONDataParser::_parseFFDFrame(const rapidjson::Value & rawData, unsigned frameStart, unsigned frameCount)
{
...
void JSONDataParser::_parseTweenFrame(const rapidjson::Value& rawData, TweenFrameData& frame, unsigned frameStart, unsigned frameCount) const
{
    _parseFrame(rawData, frame, frameStart, frameCount);
    ...
    if (rawData.HasMember(CURVE))
    {
        ...
        TweenFrameData::samplingCurve(curve, frameCount, frame.curve);
    }
}

void JSONDataParser::_parseFrame(const rapidjson::Value & rawData, FrameData & frame, unsigned frameStart, unsigned frameCount) const
{
...
void JSONDataParser::_parseTimeline(const rapidjson::Value& rawData, TimelineData& timeline, const std::function<FrameData*(const rapidjson::Value& rawData, unsigned frameStart, unsigned frameCount)>& frameParser) const
{
